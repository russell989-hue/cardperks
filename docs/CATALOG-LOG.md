...
- Earning rates for all 15 products re-read from the issuer pages and moved onto the shared spend categories (docs/CATALOG.md), with the issuer's caps and exclusions in `notes`; `currency_name` added to every product. Business Platinum's page does not list its "key business categories", so only the $5,000-purchase route is recorded. Southwest Plus: the page now shows 2X on Southwest, gas and grocery (first $5,000 combined) and Lyft; the earlier hotel/car and streaming rows are gone.
